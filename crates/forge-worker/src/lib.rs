...
use forge_protocol::{
    Frame, Heartbeat, MessageKind, TaskRequest as WireTaskRequest,
            ...
            loop {
                ...
                if started.elapsed() >= timeout {
                    ...
                    return Ok(TaskResult {
                        ...
                        stderr: timeout_stderr(&output.stderr),
...
fn timeout_stderr(stderr: &[u8]) -> String {
    let existing = String::from_utf8_lossy(stderr);
...
#[cfg(target_os = "windows")]
fn default_shell_args(command: &str) -> [&str; 2] {
    ["/C", command]
}
...
mod tests {
    ...
    fn worker_times_out_and_terminates_task() {
        ...
        let result = worker
            .execute_with_timeout(command, Some(Duration::from_millis(100)))
            .unwrap();
        assert!(!result.success);
        assert!(result.timed_out);
    ...
    fn worker_completes_fast_task_before_timeout() {
        let worker = Worker::new("local", 1);
        let result = worker
            .execute_with_timeout("echo fast", Some(Duration::from_secs(2)))
            .unwrap();
        assert!(result.success);
        assert!(!result.timed_out);
    }
