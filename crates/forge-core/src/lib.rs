...
impl fmt::Display for GraphError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingDependency { task, dependency } => write!(f, "task {task} references missing dependency {dependency}"),
...
impl TaskGraph {
    pub fn add_task(&mut self, id: TaskId, command: impl Into<String>, dependencies: Vec<TaskId>) -> Result<(), GraphError> {
        ...
        for &dependency in &dependencies {
            if dependency == id || !self.tasks.contains_key(&dependency) {
                return Err(GraphError::MissingDependency { task: id, dependency });
            }
        }

        self.tasks.insert(id, Task { id, command: command.into(), dependencies, state: TaskState::Pending });
    ...
    pub fn runnable(&self) -> Vec<TaskId> {
        self.tasks
            .values()
            .filter(|task| task.state == TaskState::Pending && task.dependencies.iter().all(|dependency| {
                self.tasks.get(dependency).is_some_and(|task| task.state == TaskState::Succeeded)
            }))
            .map(|task| task.id)
            .collect()
    }
    ...
    pub fn reconcile_blocked(&mut self) -> Vec<TaskId> {
        let blocked = self.tasks.values().filter(|task| {
            task.state == TaskState::Pending && task.dependencies.iter().any(|dependency| {
                self.tasks.get(dependency).is_some_and(|dependency| matches!(dependency.state, TaskState::Failed | TaskState::Blocked))
            })
    ...
    pub fn pending(&self) -> Vec<TaskId> {
        self.tasks.values().filter(|task| task.state == TaskState::Pending).map(|task| task.id).collect()
    }

    fn has_cycle(&self) -> bool {
        fn visit(id: TaskId, graph: &TaskGraph, visiting: &mut Vec<TaskId>, visited: &mut Vec<TaskId>) -> bool {
            ...
            visiting.push(id);
            let cycle = graph.tasks.get(&id).map(|task| {
                task.dependencies.iter().copied().any(|dependency| visit(dependency, graph, visiting, visited))
            }).unwrap_or(false);
            visiting.pop();
        ...
        self.tasks.keys().copied().any(|id| visit(id, self, &mut visiting, &mut visited))
    }
}
...
mod tests {
    ...
    fn duplicate_tasks_are_rejected() {
        let mut graph = TaskGraph::default();
        graph.add_task(1, "compile", Vec::new()).unwrap();
        assert_eq!(graph.add_task(1, "test", Vec::new()), Err(GraphError::DuplicateTask(1)));
    }
    ...
    fn task_ids_are_deterministic() {
        let mut graph = TaskGraph::default();
        graph.add_task(3, "third", Vec::new()).unwrap();
        graph.add_task(1, "first", Vec::new()).unwrap();
        graph.add_task(2, "second", Vec::new()).unwrap();
        assert_eq!(graph.task_ids().collect::<Vec<_>>(), vec![1, 2, 3]);
    }
}
